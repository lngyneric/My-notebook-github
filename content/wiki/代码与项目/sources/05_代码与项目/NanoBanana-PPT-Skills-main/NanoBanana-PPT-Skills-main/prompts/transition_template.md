---
source: raw/05_代码与项目/NanoBanana-PPT-Skills-main/NanoBanana-PPT-Skills-main/prompts/transition_template.md
raw_sha256: d97ec495263d8b4b73e59885edbeb167af6747917ea2cc1f99c3bfcda8eb3adb
compiled_at: 2026-04-14T04:09:57.723Z
---
# 首尾帧视频生成转场提示词模板

> 来源路径：`raw/05_代码与项目/NanoBanana-PPT-Skills-main/NanoBanana-PPT-Skills-main/prompts/transition_template.md`

## 基本信息
| 属性 | 说明 |
|------|------|
| 适用模型 | Kling、Veo、Seedance |
| 使用方式 | 将首帧、尾帧图片以及本提示词，发送给任意支持多模态的大语言模型 |
...
## TL;DR
这是一套用于给AI视频生成模型创作首尾帧转场描述的结构化提示词模板，通过差异分类选择对应转场策略，配合可选变化元素组合，输出符合要求的转场动态描述。

## 核心要点
1. 创作流程为三步式：差异分类 → 转场策略选择 → 具体变化构思
2. 差异分类规则：
   - A类（关联性强）：主体/场景基本一致，仅状态/风格/环境改变 → 采用**原地演变策略**，尽量不使用或仅使用微弱镜头移动
   - B类（差异巨大）：主体/场景完全不同 → 采用**运镜驱动转场策略**，必须使用明确的摄像机移动引导过渡
3. 变化构思可从三类元素自由组合：主体变化、环境变化、风格/特效变化
4. 输出要求：整合为连贯段落，描述具体直接、侧重视觉呈现，严格遵循选定的摄像机策略，避免模糊修辞
