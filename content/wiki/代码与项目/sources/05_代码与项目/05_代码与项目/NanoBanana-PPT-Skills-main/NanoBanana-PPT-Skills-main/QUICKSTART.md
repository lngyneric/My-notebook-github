...
compiled_at: 2026-04-14T04:07:33.818Z
---
# NanoBanana-PPT-Skills 快速使用指南

> 来源路径：`raw/05_代码与项目/NanoBanana-PPT-Skills-main/NanoBanana-PPT-Skills-main/QUICKSTART.md`

---

## TL;DR
本指南是 NanoBanana AI PPT 生成工具的 5 分钟快速上手教程，基于 Google Gemini API，可通过 Claude Code 或命令行，从 Markdown 文档一键生成带 HTML 播放器的AI设计PPT，支持自定义风格、分辨率和内容规划。

---

## 要点
1. **前置依赖**：需要准备 Google Gemini API 密钥，安装 `google-genai` 和 `pillow` 两个 Python 库
2. **最简流程**：准备结构化 Markdown 文档 → 在 Claude Code 发送生成指令 → 打开输出的 HTML 文件即可使用
3. **输出形式**：生成单页PPT图片+可交互HTML播放器，支持翻页、全屏、自动播放，可通过浏览器打印导出PDF
4. **支持自定义**：可自定义PPT页数、风格、分辨率，支持手动调整内容规划、批量生成多个版本
5. **规格参考**：2K分辨率约30秒/页、4K分辨率约60秒/页，日常演示推荐使用2K分辨率
...
> [!WARNING] 冲突：本页内容仅编译自当前QUICKSTART.md原始文件，未和项目其他文件做校验，若与其他文档存在矛盾请以实际代码和官方说明为准。
