...
function trySymlink(target: string, linkPath: string): void {
  try {
    ...
    // Fallback: copy directory on Windows when symlink fails (EPERM)
...
export async function installPlugin(
  ...
  if (spec.local) {
    ...
    try { fs.symlinkSync(spec.repo, pluginDir, "dir") } catch { fs.cpSync(spec.repo, pluginDir, { recursive: true, force: true }) }
