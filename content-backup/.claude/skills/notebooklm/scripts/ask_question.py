...
def ask_notebooklm(question: str, notebook_url: str, headless: bool = True, notebook_name: str = None) -> str:
    """
    ...
    Args:
        ...
        notebook_name: Name of the notebook (for fallback navigation)
    ...
    try:
        ...
        try:
            ...
            print("  ⚠️ Timeout waiting for NotebookLM domain")

        # Check for dashboard redirect
        time.sleep(2)
        if page.url.rstrip("/") == "https://notebooklm.google.com" or page.url.rstrip("/") == "https://notebooklm.google.com/notebooks":
            print("  ⚠️ Redirected to dashboard.")
            if notebook_name:
                print(f"  Trying to find notebook '{notebook_name}'...")
                try:
                    # Click on the notebook with the name
                    # Use a broad text selector
                    page.get_by_text(notebook_name).first.click()
                    print(f"  ✓ Clicked notebook '{notebook_name}'")
                    # Wait for navigation
                    page.wait_for_url(re.compile(r"notebook/"), timeout=10000)
                except Exception as e:
                    print(f"  ❌ Could not find/click notebook on dashboard: {e}")
            else:
                print("  ❌ No notebook name provided for fallback.")
        ...
        for selector in QUERY_INPUT_SELECTORS:
            try:
                ...
                if query_element:
                    ...
                    # Ensure enabled
                    if not query_element.is_enabled():
                        print("    Waiting for input to be enabled...")
                        # Simple wait loop
                        for _ in range(20):
                            time.sleep(0.5)
                            if query_element.is_enabled():
                                break
                    break
            except:
                continue
        ...
        if not query_element or not input_selector:
            ...
            # Debug: print all textareas
            textareas = page.query_selector_all('textarea')
            print(f"  Found {len(textareas)} textareas:")
            for ta in textareas:
                try:
                    print(f"    - Class: {ta.get_attribute('class')}, Placeholder: {ta.get_attribute('placeholder')}, Visible: {ta.is_visible()}")
                except:
                    pass
            return None
