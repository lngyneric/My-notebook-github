...
class TransitionPromptGenerator:
    ...
    def __init__(self, template_path: str = "prompts/transition_template.md"):
        """
        初始化生成器

        Args:
            template_path: 首尾帧提示词模板路径
        """
        ...
        # 初始化Claude客户端
        # 从环境变量读取API密钥（如果在Claude Code环境中可能不需要）
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if api_key:
            self.client = Anthropic(api_key=api_key)
        else:
            # 尝试使用默认配置（Claude Code环境）
            try:
...
"""
        ...
        try:
            response = self.client.messages.create(
                model="claude-sonnet-4-5-20250929",  # 使用最新的Claude模型
                max_tokens=2000,
                temperature=0.7,
                messages=[
                    {
...
if __name__ == "__main__":
    ...
    # 测试：如果有已生成的PPT图片，可以测试
    # prompt = generator.generate_prompt(
    #     "outputs/xxx/images/slide-01.png",
    #     "outputs/xxx/images/slide-02.png",
    #     content_context="从封面页过渡到第一个内容页"
    # )
